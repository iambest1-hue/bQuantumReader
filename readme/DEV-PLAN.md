...
│   │   └── ModelManager.kt            # 模型文件下载管理
├── domain/
│   ├── MarkdownGen.kt                 # Markdown 生成器
│   ├── LinkParser.kt                  # URL 解析（提取 bvid）
│   └── WhisperEngine.kt               # whisper.cpp JNI 封装
├── cpp/
│   ├── CMakeLists.txt                 # NDK 编译配置
│   ├── whisper.h / whisper.cpp        # whisper.cpp 源码（submodule）
│   └── whisper_jni.cpp                # JNI 桥接函数
├── util/
...
| 任务 | 产出 |
|------|------|
| whisper.cpp NDK 集成 | CMake 编译 .so，封装 JNI 接口 |
| WhisperEngine | Kotlin 层调用 JNI，管理模型加载与转写 |
| ModelManager | 首次启动下载模型（small/medium），进度显示 |
| 语音识别流程 | 获取音频 URL → 下载到本地 → 传入 whisper 转写 → 显示结果 |
| ProgressSection | 下载进度 + 转写进度 + 实时计时 + 取消按钮 |
...
NDK 集成需要在 `build.gradle.kts` 中添加：
```kotlin
android {
...
3. 点击语音识别 → 下载模型（首次）→ 下载音频 → 本地转写 → 显示结果
