...
## asr.js → WhisperClient.kt

**HTTP 协议（简单，直接调用）：**

### 检测连接
```
GET http://{serverUrl}/status
Response: { "ready": true, "model": "small", "device": "cpu" }
```

### 发送转写
```
POST http://{serverUrl}/transcribe
Content-Type: application/json
Body: { "audio_url": "https://...", "language": "zh" }
Response: { "segments": [{ "from": 0, "to": 3500, "content": "文本" }], "duration": 120000 }
```
...
整个 `whisper_server/` 目录在电脑端直接使用，不涉及安卓。安卓 APP 通过 HTTP 调用即可。
